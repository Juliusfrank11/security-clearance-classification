...
LEARNING_RATES = [0.001, 0.1, 0.000000001]
DROPOUT_RATES = [0.5, 0.1, 0.001]
...
for LEARNING_RATE in LEARNING_RATES:
    for DROPOUT_RATE in DROPOUT_RATES:
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            for batch_num, batch in enumerate(train_loader):
                ...
                print(
                    "A",
                    torch.eq(
                        torch.round(torch.sigmoid(predictions.cpu())).int(), targets
                    )
                    .float()
                    .mean(),
                    "\tBatch",
            ...
            if accuracy_score(test_y_true, test_y_pred) <= last_epoch_accuracy and epoch > NUM_EPOCHS//2:
                break
            last_epoch_accuracy = accuracy_score(test_y_true, test_y_pred)
        model_suffex = (
            str(LEARNING_RATE).replace(".", "")
            + "-"
            + str(DROPOUT_RATE).replace(".", "")
        )
        torch.save(model, f"LSTM-{model_suffex}")

        eval_approve_texts, eval_approve_labels = load_text_files(
            r"data/finding_of_fact/eval/approve"
        )
        eval_deny_texts, eval_deny_labels = load_text_files(
        ...
        eval_labels = np.array(
            list(
                map(
                    lambda x: 0 if x.endswith("approve") else 1,
                    eval_approve_labels + eval_deny_labels,
                )
            )
        )
        ...
        for batch in eval_loader:
            ...
            eval_text_split_idx_padded = nn.utils.rnn.pad_sequence(
                eval_text_split_idx, batch_first=True
            ).int()
            eval_text_split_idx_padded = eval_text_split_idx_padded.to(device)
            eval_text_lengths = torch.tensor(
                [len(text) for text in eval_text_split_idx_padded], dtype=torch.int32
            )
        ...
        with open("lstm_model_results.csv", "a") as f:
            f.write(
                "\n"
                + ",".join(
                    [
